 ...
 *
 * ---------------------------------------------------------------------------
 * For mc-render the authoritative row is
 *   mc-meshing, mc-sim, mc-worldgen   (+ mc-kernel, universally)
 *
 * mc-physics and mc-save are reachable transitively through mc-sim and are
 * therefore forbidden (rule 3): the renderer draws what the simulation says is
 * true, and never re-runs a collision query or reads a save file to find out.
 *
 * mc-playground-kit is absent from `dependencies` and must stay absent. This
 * repository owns the runtime input service (plan.md §2.3-2); if input were
 * taken from the playground harness instead, the shipped game would have no
 * input at all, because the harness is dev-only and is never published into a
 * release build.
 * ---------------------------------------------------------------------------
 */
